...
<div class="space-y-4">
    ...
    {{-- Mini Cards de Resumo - Layout compacto em linha --}}
    <div class="grid grid-cols-4 gap-3">
        ...
        <a href="{{ route('admin.documentos-pendentes.index') }}" class="bg-white rounded-lg border {{ $docsAtrasados > 0 ? 'border-red-300 bg-red-50/50' : 'border-gray-200' }} p-3 hover:border-purple-300 hover:shadow transition-all group">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 {{ $docsAtrasados > 0 ? 'bg-red-100' : 'bg-purple-100' }} rounded-lg flex items-center justify-center group-hover:{{ $docsAtrasados > 0 ? 'bg-red-200' : 'bg-purple-200' }} transition flex-shrink-0">
                    <svg class="w-4 h-4 {{ $docsAtrasados > 0 ? 'text-red-600' : 'text-purple-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div class="xl:col-span-2 space-y-4">
            ...
            {{-- Minhas Tarefas - Seção Principal --}}
            @php
                $tarefasArray = [];
                foreach($documentos_pendentes_aprovacao ?? [] as $doc) {
                ...
                $tarefasAgrupadas = collect($tarefasArray)->sortByDesc('dias_pendente')->take(8);
            ...
            <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                    ...
                    @if($totalTarefas > 0 || count($ordens_servico_andamento ?? []) > 0)
                    <span class="text-xs text-gray-500">{{ $totalTarefas + count($ordens_servico_andamento ?? []) }} pendente(s)</span>
                    @endif
                </div>
                
                <div class="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
                    {{-- Assinaturas Pendentes --}}
                    @foreach(($documentos_pendentes_assinatura ?? [])->take(3) as $assinatura)
                    <div class="px-4 py-2.5 flex items-center gap-3 hover:bg-gray-50 transition">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-500">Assinatura • {{ $assinatura->created_at->diffForHumans() }}</p>
                        </div>
                        <a href="{{ route('admin.assinatura.assinar', $assinatura->documentoDigital->id) }}" class="px-2.5 py-1 bg-amber-500 text-white text-xs font-medium rounded hover:bg-amber-600 transition">
                            Assinar
                        </a>
                    </div>
                    @endforeach
                    ...
                    @foreach(($ordens_servico_andamento ?? collect())->take(5) as $os)
                    @php
                        $diasRestantesOS = $os->data_fim ? now()->startOfDay()->diffInDays($os->data_fim->startOfDay(), false) : null;
                        $isVencidoOS = $diasRestantesOS !== null && $diasRestantesOS < 0;
                        $isHojeOS = $diasRestantesOS === 0;
                        $isVencendoOS = $diasRestantesOS !== null && $diasRestantesOS > 0 && $diasRestantesOS <= 3;
                    @endphp
                    <a href="{{ route('admin.ordens-servico.show', $os) }}" class="block px-4 py-2.5 hover:bg-gray-50 transition {{ $isVencidoOS ? 'bg-red-50/50' : '' }}">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 {{ $isVencidoOS ? 'bg-red-100' : 'bg-blue-100' }} rounded-lg flex items-center justify-center flex-shrink-0">
                                <svg class="w-4 h-4 {{ $isVencidoOS ? 'text-red-600' : 'text-blue-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/>
                                </svg>
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    <p class="text-sm font-medium text-gray-900">OS #{{ $os->numero }}</p>
                                    <span class="px-1.5 py-0.5 text-[10px] font-medium rounded bg-blue-100 text-blue-700">OS</span>
                                </div>
                                <p class="text-xs text-gray-500 truncate">
                                    {{ $os->estabelecimento->nome_fantasia ?? 'Sem estabelecimento' }}
                                    @php $tiposAcaoOS = $os->tiposAcao(); @endphp
                                    @if($tiposAcaoOS && $tiposAcaoOS->count() > 0)
                                        • {{ $tiposAcaoOS->first()->descricao }}
                                    @endif
                                </p>
                            </div>
                            <div class="flex items-center gap-1.5 flex-shrink-0">
                                @if($os->data_fim)
                                    @if($isVencidoOS)
                                        <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-red-100 text-red-700">{{ abs($diasRestantesOS) }}d atrasado</span>
                                    @elseif($isHojeOS)
                                        <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-orange-100 text-orange-700">Hoje</span>
                                    @elseif($isVencendoOS)
                                        <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-amber-100 text-amber-700">{{ $diasRestantesOS }}d</span>
                                    @else
                                        <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-700">{{ $diasRestantesOS }}d</span>
                                    @endif
                                @else
                    ...
                    @php $diasRestantes = 5 - $tarefa['dias_pendente']; @endphp
                    <a href="{{ route('admin.estabelecimentos.processos.show', [$tarefa['estabelecimento_id'], $tarefa['processo_id']]) }}" 
                       class="block px-4 py-2.5 hover:bg-gray-50 transition {{ $tarefa['atrasado'] ? 'bg-red-50/50' : '' }}">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 {{ $tarefa['atrasado'] ? 'bg-red-100' : 'bg-purple-100' }} rounded-lg flex items-center justify-center flex-shrink-0 relative">
                                <svg class="w-4 h-4 {{ $tarefa['atrasado'] ? 'text-red-600' : 'text-purple-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                @if($tarefa['total'] > 1)
                                <span class="absolute -top-1 -right-1 w-4 h-4 bg-{{ $tarefa['atrasado'] ? 'red' : 'purple' }}-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">{{ $tarefa['total'] }}</span>
                                @endif
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    <p class="text-sm font-medium text-gray-900 truncate">{{ Str::limit($tarefa['primeiro_arquivo'], 30) }}</p>
                                    @if($tarefa['total'] > 1)
                                    <span class="text-xs text-gray-400">+{{ $tarefa['total'] - 1 }}</span>
                                    @endif
                                </div>
                                <p class="text-xs text-gray-500 truncate">{{ $tarefa['estabelecimento'] }} • {{ $tarefa['numero_processo'] }}</p>
                            </div>
                            <div class="flex items-center gap-1.5 flex-shrink-0">
                                @if($tarefa['atrasado'])
                                    <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-red-100 text-red-700">{{ $tarefa['dias_pendente'] - 5 }}d atrasado</span>
                                @else
                                    <span class="px-2 py-0.5 text-xs font-medium rounded-full {{ $diasRestantes <= 1 ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700' }}">{{ $diasRestantes }}d</span>
                                @endif
                                <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                </svg>
                    ...
                    @if(count($documentos_pendentes_assinatura ?? []) == 0 && $tarefasAgrupadas->isEmpty() && ($ordens_servico_andamento ?? collect())->isEmpty())
                ...
                <div class="px-4 py-2 bg-gray-50 border-t border-gray-100">
                    <a href="{{ route('admin.documentos-pendentes.index') }}" class="text-xs text-purple-600 hover:text-purple-700 font-medium">Ver todas as tarefas →</a>
                </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                    ...
                    <span class="px-2 py-0.5 bg-cyan-100 text-cyan-700 text-xs font-medium rounded-full">{{ $stats['processos_atribuidos'] ?? count($processos_atribuidos) }}</span>
                </div>
                <div class="divide-y divide-gray-100 max-h-[250px] overflow-y-auto">
                    @foreach(collect($processos_atribuidos)->take(5) as $proc)
                    @php $isMeuDireto = $proc->responsavel_atual_id === auth('interno')->id(); @endphp
                    <a href="{{ route('admin.estabelecimentos.processos.show', [$proc->estabelecimento_id, $proc->id]) }}" class="block px-4 py-2.5 hover:bg-gray-50 transition">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 {{ $isMeuDireto ? 'bg-cyan-100' : 'bg-blue-100' }} rounded-lg flex items-center justify-center flex-shrink-0">
                                <svg class="w-4 h-4 {{ $isMeuDireto ? 'text-cyan-600' : 'text-blue-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="px-1.5 py-0.5 text-[10px] font-medium rounded {{ $isMeuDireto ? 'bg-cyan-100 text-cyan-700' : 'bg-blue-100 text-blue-700' }}">{{ $isMeuDireto ? 'Meu' : 'Setor' }}</span>
                                </div>
                            ...
                            <span class="px-2 py-0.5 text-xs font-medium rounded-full
                                ...
                                @else bg-gray-100 text-gray-700 @endif">{{ $proc->status_nome }}</span>
                        </div>
                    </a>
                    @endforeach
                </div>
                @if(($stats['processos_atribuidos'] ?? 0) > 5)
                <div class="px-4 py-2 bg-gray-50 border-t border-gray-100">
                    <a href="{{ route('admin.processos.index-geral', ['responsavel' => 'meus']) }}" class="text-xs text-cyan-600 hover:text-cyan-700 font-medium">Ver todos →</a>
                </div>
                @endif
            </div>
            @endif

            {{-- Grid de 2 colunas para Cadastros e Prazos --}}
                ...
                <div class="divide-y divide-blue-400/20">
                    ...
                    <div class="group relative">
                        ...
                        <div class="absolute right-2 top-1/2 -translate-y-1/2 hidden group-hover:flex gap-1">
                            <button @click.prevent="editarAtalho({{ $atalho->id }}, '{{ addslashes($atalho->titulo) }}', '{{ addslashes($atalho->url) }}', '{{ $atalho->icone }}')" class="p-1 text-blue-200 hover:text-white" title="Editar">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                            </button>
                            <button @click.prevent="excluirAtalho({{ $atalho->id }})" class="p-1 text-blue-200 hover:text-red-300" title="Excluir">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </button>
                        </div>
                    </div>
                    @empty
                    <a href="{{ route('admin.documentos-pendentes.index') }}" class="flex items-center gap-3 px-4 py-2 hover:bg-blue-400/20 transition">
                        <span class="text-sm text-white">Documentos Pendentes</span>
                    </a>
                    <a href="{{ route('admin.estabelecimentos.pendentes') }}" class="flex items-center gap-3 px-4 py-2 hover:bg-blue-400/20 transition">
                        <span class="text-sm text-white">Estabelecimentos Pendentes</span>
                    </a>
                    <a href="{{ route('admin.assinatura.pendentes') }}" class="flex items-center gap-3 px-4 py-2 hover:bg-blue-400/20 transition">
                    ...
                    <div @click.away="fecharModal()" class="bg-white rounded-xl shadow-xl w-full max-w-md text-gray-900">
                        ...
                        <form @submit.prevent="salvarAtalho()" class="p-6 space-y-4">
                            <div>
                                ...
                                <select x-model="form.url" @change="atualizarTituloPadrao()" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <div>
                                ...
                                <input type="text" x-model="form.titulo" required maxlength="100" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Ex: Meus Processos">
                            </div>
                            <div>
                            ...
                            <div class="flex gap-3 pt-2">
                                <button type="button" @click="fecharModal()" class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">Cancelar</button>
    ...
    return {
        ...
        async salvarAtalho() {
            ...
            try {
                ...
                if (data.success) { window.location.reload(); } else { alert(data.error || 'Erro ao salvar atalho'); }
            } catch (e) { alert('Erro ao salvar atalho'); }
            this.salvando = false;
        },
        async excluirAtalho(id) {
            ...
            } catch (e) { alert('Erro ao excluir atalho'); }
        }
    }
}
</script>
