            ...
            <div class="divide-y divide-gray-50 min-h-[200px] max-h-[320px] overflow-y-auto scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-100 hover:scrollbar-thumb-blue-600">
                ...
                <template x-if="!loading && tarefas.length > 0">
                    <div>
                        ...
                        {{-- Seção: Documentos do Setor (Aprovações e Respostas) --}}
                        <template x-if="tarefas.filter(t => t.tipo === 'aprovacao' || t.tipo === 'resposta').length > 0">
                            <div>
                                ...
                                <template x-for="t in tarefas.filter(t => t.tipo === 'aprovacao' || t.tipo === 'resposta')" :key="'setor-' + t.tipo + (t.id || t.processo_id)">
                                    <a :href="t.url" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition relative" :class="t.atrasado ? 'bg-red-50/50' : ''">
                                        <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" :class="t.tipo === 'resposta' ? (t.atrasado ? 'bg-red-100' : 'bg-green-100') : (t.atrasado ? 'bg-red-100' : 'bg-purple-100')">
                                    ...
                                    <p class="text-sm font-medium text-gray-900 flex items-center gap-2 flex-wrap">
                                        ...
                                        <template x-if="p.docs_total > 0">
                                            <span class="text-[10px] font-medium px-1.5 py-0.5 rounded-full flex items-center gap-1"
                                                  :class="p.docs_enviados >= p.docs_total ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'"
                                                  :title="'Documentos obrigatórios: ' + p.docs_enviados + ' de ' + p.docs_total + ' enviados'">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                                <span x-text="p.docs_enviados + '/' + p.docs_total"></span>
                                            </span>
                                        </template>
                                        {{-- Documentos pendentes de aprovação --}}
                                        <template x-if="p.docs_pendentes > 0">
                                            <span class="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-yellow-100 text-yellow-700"
                                                  :title="p.docs_pendentes + ' documento(s) aguardando aprovação'">
                                                +<span x-text="p.docs_pendentes"></span> pend.
                                            </span>
                                        </template>
