...
@section('title', 'Documentos IA')
@section('page-title', 'Documentos IA')
...
<div class="space-y-6" x-data="{ activeTab: '{{ $tab }}' }">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <template x-if="activeTab === 'documentos'">
            ...
            <template x-if="activeTab === 'categorias'">
    ...
    <div class="border-b border-gray-200">
        <nav class="-mb-px flex space-x-8">
            <button @click="activeTab = 'documentos'" 
                ...
                Documentos
                <span class="bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full text-xs font-semibold">{{ $documentos->total() }}</span>
            </button>
            <button @click="activeTab = 'categorias'" 
                    :class="activeTab === 'categorias' ? 'border-rose-500 text-rose-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center gap-2 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                </svg>
                Categorias
                <span class="bg-rose-100 text-rose-600 px-2 py-0.5 rounded-full text-xs font-semibold">{{ $categorias->total() }}</span>
            </button>
        </nav>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            @if($categorias->isEmpty())
                <div class="p-12 text-center">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Comece criando uma nova categoria para organizar seus documentos.</p>
                    <div class="mt-6">
                        <a href="{{ route('admin.configuracoes.categorias-pops.create') }}" 
                           class="inline-flex items-center gap-2 px-4 py-2 bg-rose-600 text-white text-sm font-semibold rounded-lg hover:bg-rose-700 transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                ...
                <div class="px-6 py-4 border-t border-gray-200">
                    {{ $categorias->appends(['tab' => 'categorias'])->links() }}
                </div>
                @endif
            @endif
        </div>
