    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-4">
        <div class="xl:col-span-1 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div class="flex items-start justify-between gap-3 mb-4">
                ...
                <span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-700 font-semibold">Novo fluxo</span>
            </div>
            ...
            @if($convitesCadastro->isNotEmpty())
                <div class="divide-y divide-gray-200">
                    @foreach($convitesCadastro as $convite)
                        ...
                        <div class="p-4" x-data="{ showQr: false }">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="mt-2 flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">
                                        <code class="text-xs text-gray-700 truncate flex-1">{{ $linkCadastro }}</code>
                                        <button type="button"
                                    ...
                                    <div class="mt-2 flex items-center gap-2 flex-wrap">
                                        <button type="button" @click="showQr = !showQr"
                                            ...
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM17 14v3m0 3h.01M14 17h3"/>
                                            </svg>
                                            <span x-text="showQr ? 'Fechar QR' : 'QR Code'"></span>
                                        </button>
                            ...
                            {{-- QR Code expandível --}}
                            <div x-show="showQr" x-transition class="mt-3 pt-3 border-t border-gray-100">
                                <div class="flex flex-col items-center gap-2">
                                    <div class="bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
                                        @if(!empty($qrCodes[$convite->id]))
                                            <img src="data:image/png;base64,{{ $qrCodes[$convite->id] }}" alt="QR Code" class="w-48 h-48">
                                        @else
                                            <p class="text-sm text-red-500">Erro ao gerar QR Code.</p>
                                        @endif
                                    </div>
                                    <p class="text-xs text-gray-500">Aponte a câmera do celular para acessar o cadastro</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
