...
@section('page-title', 'Pendências e Alertas')

@section('content')
@php
    $totalDocumentos = $documentosComPrazo->count() + $documentosRejeitados->count() + $documentosPendentes->count();
@endphp

<div class="space-y-6">
    {{-- Cabeçalho --}}
    <div>
        <p class="text-sm text-gray-500">Acompanhe todas as pendências dos seus estabelecimentos e processos em um só lugar</p>
    </div>

    {{-- ============================================================== --}}
    {{-- SEÇÃO ÚNICA: ALERTAS DE PROCESSOS                              --}}
    {{-- ============================================================== --}}
    <div>
        <div class="flex items-center gap-2 mb-4">
            ...
            <div>
                <h2 class="text-base font-bold text-gray-900">Alertas de Processos</h2>
                <p class="text-xs text-gray-500">Alertas e cobranças criados pela Vigilância Sanitária nos seus processos</p>
            </div>
        </div>

        {{-- Alertas de Documentos (dentro da seção) --}}
        @if($totalDocumentos > 0)
        <div class="space-y-4 mb-6">
            {{-- Notificações com Prazo Pendente --}}
            ...
            <div class="bg-white rounded-xl shadow-sm border border-amber-300 overflow-hidden">
                <div class="px-4 py-3 border-b border-amber-200 bg-amber-50 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-sm font-semibold text-amber-800">Notificações com Prazo para Resposta</h3>
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs font-semibold mt-0.5 {{ $documento->vencido ? 'text-red-600' : ($documento->dias_faltando <= 7 ? 'text-amber-600' : 'text-green-600') }}">
                            ...
                            <a href="{{ route('company.processos.show', $documento->processo_id) }}" 
                               class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white {{ $documento->vencido ? 'bg-red-600 hover:bg-red-700' : 'bg-amber-600 hover:bg-amber-700' }} rounded-lg transition-colors flex-shrink-0">
            ...
            {{-- 1.2 Documentos Rejeitados --}}
            @if($documentosRejeitados->count() > 0)
            <div class="bg-white rounded-xl shadow-sm border border-red-300 overflow-hidden">
                <div class="px-4 py-3 border-b border-red-200 bg-red-50 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-sm font-semibold text-red-800">Documentos Rejeitados - Correção Necessária</h3>
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-red-600 mt-0.5 line-clamp-2">
                            ...
                            <a href="{{ route('company.processos.show', $documento->processo_id) }}" 
                               class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors flex-shrink-0">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {{-- 1.3 Documentos Pendentes de Visualização --}}
            @if($documentosPendentes->count() > 0)
            <div class="bg-white rounded-xl shadow-sm border border-blue-200 overflow-hidden">
                <div class="px-4 py-3 border-b border-blue-100 bg-blue-50 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <svg class="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        </svg>
                        <h3 class="text-sm font-semibold text-blue-800">Documentos Pendentes de Visualização</h3>
        ...
        {{-- Estatísticas dos Alertas de Processo --}}
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="px-6 py-12 text-center">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum alerta de processo encontrado</h3>
                <p class="mt-1 text-sm text-gray-500">Os alertas criados pela Vigilância Sanitária nos seus processos serão exibidos aqui.</p>
            </div>
            @endif
        </div>
    </div>
