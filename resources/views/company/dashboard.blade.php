...
<div class="space-y-6">
    ...
    <div x-data="{ mostrarAviso: localStorage.getItem('ocultarAvisoInfovisa') !== 'true' }" x-show="mostrarAviso" x-cloak>
        <div class="bg-amber-50 border-l-4 border-amber-400 rounded-lg p-3 shadow-sm relative">
            {{-- Botão Fechar --}}
            <button 
                @click="localStorage.setItem('ocultarAvisoInfovisa', 'true'); mostrarAviso = false"
                class="absolute top-2 right-2 p-1 text-amber-600 hover:text-amber-800 transition-colors"
            ...
            <div class="flex items-start gap-2 pr-6">
                ...
                <div class="flex-1">
                    <p class="text-xs font-semibold text-amber-900 mb-1">
                        Processos até 2025: acesse <a href="https://sistemas.saude.to.gov.br/infovisa2/" target="_blank" class="text-blue-600 hover:underline">sistema antigo</a>
                    </p>
                    <p class="text-xs text-amber-800">Processos a partir de 2026: utilize este sistema (InfoVISA 3.0)</p>
                </div>
            </div>
        </div>
    </div>
