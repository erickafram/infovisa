...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">
                Editar Responsável {{ $tipo === 'tecnico' ? 'Técnico' : 'Legal' }}
            </h1>
    ...
    @if(($tipo === 'legal' && !$responsavel->documento_identificacao) || ($tipo === 'tecnico' && !$responsavel->carteirinha_conselho))
    <div class="mb-6 bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-lg">
        <div class="flex items-start">
            ...
            <div>
                <p class="text-sm font-medium text-yellow-800">Documento pendente</p>
                <p class="text-sm text-yellow-700 mt-1">
                    @if($tipo === 'tecnico')
                        A carteirinha do conselho é obrigatória para Responsável Técnico. Por favor, faça o upload abaixo.
                    @else
                        O documento de identificação é obrigatório para Responsável Legal. Por favor, faça o upload abaixo.
                    @endif
                </p>
            </div>
        </div>
    ...
    {{-- Formulário --}}
    <form action="{{ route('company.estabelecimentos.responsaveis.update', [$estabelecimento->id, $responsavel->id]) }}" 
          method="POST" 
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-base font-mono text-gray-900">{{ $responsavel->cpf_formatado ?? $responsavel->cpf }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="telefone" id="telefone" value="{{ old('telefone', $responsavel->telefone) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="conselho" id="conselho" required
                            ...
                            <option value="CRF" {{ old('conselho', $responsavel->conselho) == 'CRF' ? 'selected' : '' }}>CRF - Conselho Regional de Farmácia</option>
                            <option value="CRM" {{ old('conselho', $responsavel->conselho) == 'CRM' ? 'selected' : '' }}>CRM - Conselho Regional de Medicina</option>
                            <option value="CRMV" {{ old('conselho', $responsavel->conselho) == 'CRMV' ? 'selected' : '' }}>CRMV - Conselho Regional de Medicina Veterinária</option>
                            <option value="CRO" {{ old('conselho', $responsavel->conselho) == 'CRO' ? 'selected' : '' }}>CRO - Conselho Regional de Odontologia</option>
                            <option value="COREN" {{ old('conselho', $responsavel->conselho) == 'COREN' ? 'selected' : '' }}>COREN - Conselho Regional de Enfermagem</option>
                            <option value="CRN" {{ old('conselho', $responsavel->conselho) == 'CRN' ? 'selected' : '' }}>CRN - Conselho Regional de Nutrição</option>
                            <option value="CRBIO" {{ old('conselho', $responsavel->conselho) == 'CRBIO' ? 'selected' : '' }}>CRBIO - Conselho Regional de Biologia</option>
                            <option value="CRQ" {{ old('conselho', $responsavel->conselho) == 'CRQ' ? 'selected' : '' }}>CRQ - Conselho Regional de Química</option>
                            <option value="CREA" {{ old('conselho', $responsavel->conselho) == 'CREA' ? 'selected' : '' }}>CREA - Conselho Regional de Engenharia</option>
                            <option value="OUTRO" {{ old('conselho', $responsavel->conselho) == 'OUTRO' ? 'selected' : '' }}>Outro</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="numero_registro" id="numero_registro" 
                               value="{{ old('numero_registro', $responsavel->numero_registro_conselho) }}" required
                               class="w-full px-4 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-mono transition-colors"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="bg-green-50 border border-green-200 rounded-lg p-4 flex items-start gap-3 mb-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-green-700 mt-1">Caso deseje substituir, faça upload de um novo arquivo abaixo.</p>
                            </div>
                        </div>
                        @endif
                        
                        <label for="carteirinha_conselho" class="block text-sm font-medium text-gray-700 mb-2">
                            {{ $responsavel->carteirinha_conselho ? 'Substituir Carteirinha do Conselho' : 'Carteirinha do Conselho' }} (PDF ou Imagem)
                            @if(!$responsavel->carteirinha_conselho)
                        ...
                        <input type="file" name="carteirinha_conselho" id="carteirinha_conselho"
                               ...
                               {{ !$responsavel->carteirinha_conselho ? 'required' : '' }}
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <label for="documento_identificacao" class="block text-sm font-medium text-gray-700 mb-2">
                    {{ $responsavel->documento_identificacao ? 'Substituir Documento' : 'RG ou CNH' }} (PDF ou Imagem)
                    @if(!$responsavel->documento_identificacao)
                    <span class="text-red-500">*</span>
                    @endif
                </label>
                <input type="file" name="documento_identificacao" id="documento_identificacao"
                       accept=".pdf,.jpg,.jpeg,.png"
                       {{ !$responsavel->documento_identificacao ? 'required' : '' }}
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
            ...
            <button type="submit"
                    class="px-6 py-2.5 text-sm font-medium text-white {{ $tipo === 'tecnico' ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700' }} rounded-lg transition-colors flex items-center gap-2">
