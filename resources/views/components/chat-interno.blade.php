...
<div x-data="chatInterno()" x-init="init()" class="fixed bottom-6 z-40" style="right: 100px;">
    ...
    <button @click="toggleChat()" 
        ...
        <span x-show="totalNaoLidas > 0 && !isOpen" x-cloak
              ...
              x-text="totalNaoLidas > 9 ? '9+' : totalNaoLidas"></span>
    </button>
        ...
        <div class="flex-1 overflow-hidden flex flex-col">
            ...
            <div x-show="!conversaAtual && !suporteAberto" class="flex-1 overflow-y-auto flex flex-col">
                ...
                <div class="flex border-b border-gray-200 bg-gray-50 flex-shrink-0">
                    <button @click="tab = 'conversas'" 
                            :class="tab === 'conversas' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500'"
                            class="flex-1 py-2 text-sm font-medium border-b-2 transition">Conversas</button>
                    <button @click="tab = 'usuarios'; carregarUsuarios()" 
                            :class="tab === 'usuarios' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500'"
                            class="flex-1 py-2 text-sm font-medium border-b-2 transition">Usuários</button>
                </div>

                {{-- Busca de Usuários --}}
                <div x-show="tab === 'usuarios'" class="p-2 border-b border-gray-100 flex-shrink-0">
                    <div class="relative">
                        <input type="text" x-model="buscaUsuario" @input.debounce.300ms="buscarUsuarios()"
                               placeholder="Buscar usuário..."
                               class="w-full pl-9 pr-4 py-2 text-sm border border-gray-300 rounded-full focus:ring-2 focus:ring-green-500 focus:border-green-500">
                ...
                <div x-show="tab === 'conversas'" class="flex-1 overflow-y-auto divide-y divide-gray-100">
                    <div x-show="conversas.length === 0" class="p-8 text-center text-gray-500">
                        ...
                        <button @click="abrirConversa(conv)" class="w-full p-3 flex items-center gap-3 hover:bg-gray-50 transition text-left">
                            <div class="relative flex-shrink-0">
                                <div class="w-12 h-12 bg-gradient-to-br from-gray-400 to-gray-500 rounded-full flex items-center justify-center">
                                    <span class="text-white font-semibold" x-text="conv.iniciais"></span>
                                </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between">
                                    <span class="font-medium text-gray-900 truncate" x-text="conv.nome"></span>
                                    <span class="text-xs text-gray-400" x-text="conv.ultima_mensagem?.data || ''"></span>
                                </div>
                                <div class="flex items-center justify-between mt-0.5">
                                    <p class="text-sm text-gray-500 truncate" x-text="conv.ultima_mensagem?.conteudo || 'Sem mensagens'"></p>
                                    <span x-show="conv.nao_lidas > 0" class="ml-2 w-5 h-5 bg-green-500 text-white text-xs font-bold rounded-full flex items-center justify-center" x-text="conv.nao_lidas"></span>
                                </div>
                        ...
                        <button @click="iniciarConversa(usuario)" class="w-full p-3 flex items-center gap-3 hover:bg-gray-50 transition text-left">
                            <div class="relative flex-shrink-0">
                                <div class="w-12 h-12 bg-gradient-to-br from-blue-400 to-blue-500 rounded-full flex items-center justify-center">
                                    <span class="text-white font-semibold" x-text="usuario.iniciais"></span>
                                </div>
            ...
            <div x-show="suporteAberto" class="flex-1 flex flex-col overflow-hidden">
                <div class="flex-1 overflow-y-auto p-3 space-y-2 bg-[#e5ddd5]">
                    ...
                    <div x-show="suporteMensagens.length === 0" class="text-center text-gray-500 py-8">
                        <p class="text-sm">Nenhum aviso do suporte</p>
                    </div>
                </div>
                <div class="p-3 bg-gray-100 border-t text-center text-xs text-gray-500">
                    Este canal é apenas para avisos. Não é possível enviar mensagens.
                </div>
            </div>
            ...
            <div x-show="conversaAtual && !suporteAberto" class="flex-1 flex flex-col overflow-hidden">
                    ...
                    <template x-for="msg in mensagens" :key="msg.id">
                        <div :class="msg.minha ? 'flex justify-end' : 'flex justify-start'" class="group">
                            <div :class="msg.minha ? 'bg-[#dcf8c6]' : 'bg-white'" class="max-w-[80%] px-3 py-2 rounded-lg shadow relative">
                                {{-- Botão apagar (só para minhas mensagens não deletadas dentro de 30min) --}}
                                <button x-show="msg.minha && msg.pode_deletar && !msg.deletada" 
                                        @click="apagarMensagem(msg.id)"
                                        class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-xs hover:bg-red-600">
                                ...
                                <div class="flex items-center justify-end gap-1 mt-1">
                                    ...
                                    <span x-show="msg.minha && !msg.deletada" class="flex items-center">
                                        <svg x-show="msg.lida" class="w-4 h-4 text-[#53bdeb]" viewBox="0 0 16 15" fill="currentColor">
                                            <path d="M15.01 3.316l-.478-.372a.365.365 0 0 0-.51.063L8.666 9.88a.32.32 0 0 1-.484.032l-.358-.325a.32.32 0 0 0-.484.032l-.378.48a.418.418 0 0 0 .036.54l1.32 1.267a.32.32 0 0 0 .484-.034l6.272-8.048a.366.366 0 0 0-.064-.512zm-4.1 0l-.478-.372a.365.365 0 0 0-.51.063L4.566 9.88a.32.32 0 0 1-.484.032L1.892 7.77a.366.366 0 0 0-.516.005l-.423.433a.364.364 0 0 0 .006.514l3.255 3.185a.32.32 0 0 0 .484-.033l6.272-8.048a.365.365 0 0 0-.063-.51z"/>
                                        </svg>
                                        <svg x-show="!msg.lida && msg.entregue" class="w-4 h-4 text-gray-400" viewBox="0 0 16 15" fill="currentColor">
                                            <path d="M15.01 3.316l-.478-.372a.365.365 0 0 0-.51.063L8.666 9.88a.32.32 0 0 1-.484.032l-.358-.325a.32.32 0 0 0-.484.032l-.378.48a.418.418 0 0 0 .036.54l1.32 1.267a.32.32 0 0 0 .484-.034l6.272-8.048a.366.366 0 0 0-.064-.512zm-4.1 0l-.478-.372a.365.365 0 0 0-.51.063L4.566 9.88a.32.32 0 0 1-.484.032L1.892 7.77a.366.366 0 0 0-.516.005l-.423.433a.364.364 0 0 0 .006.514l3.255 3.185a.32.32 0 0 0 .484-.033l6.272-8.048a.365.365 0 0 0-.063-.51z"/>
                                        </svg>
                                        <svg x-show="!msg.lida && !msg.entregue" class="w-4 h-4 text-gray-400" viewBox="0 0 16 15" fill="currentColor">
                ...
                <div class="p-3 bg-gray-100 border-t border-gray-200 flex-shrink-0">
                    <form @submit.prevent="enviarMensagem()" class="flex items-end gap-2">
                        ...
                        <div class="flex-1">
                            <input type="text" x-model="novaMensagem" @keydown.enter.prevent="enviarMensagem()"
                                   placeholder="Digite uma mensagem..."
                                   class="w-full px-4 py-2 border border-gray-300 rounded-full focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm bg-white">
                        </div>
    ...
    return {
        ...
        async carregarConversas() {
            try {
                ...
                this.conversas = await r.json();
                this.totalNaoLidas = this.conversas.reduce((t, c) => t + c.nao_lidas, 0);
        ...
        async carregarUsuarios() {
            try {
                ...
                this.usuariosOnline = this.usuarios.filter(u => u.online).length;
            } catch (e) { console.error(e); }
        },

        async buscarUsuarios() {
            try {
                const url = this.buscaUsuario 
                    ? `{{ route("admin.chat.usuarios.buscar") }}?q=${encodeURIComponent(this.buscaUsuario)}`
                    : '{{ route("admin.chat.usuarios") }}';
        ...
        async enviarMensagem() {
            ...
            try {
                ...
                const r = await fetch('{{ route("admin.chat.enviar") }}', {
                    method: 'POST',
        ...
        async apagarMensagem(msgId) {
            ...
            try {
                const r = await fetch(`{{ url('/admin/chat/mensagem') }}/${msgId}`, {
                    method: 'DELETE',
                    headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
                });
                const data = await r.json();
                if (data.success) {
                    ...
                    if (idx !== -1) {
                        this.mensagens[idx].deletada = true;
                        this.mensagens[idx].conteudo = null;
                        this.mensagens[idx].pode_deletar = false;
                    }
                } else {
        ...
        async verificarNovas() {
            try {
                ...
                if (this.conversaAtual) params.append('usuario_id', this.conversaAtual.usuario_id);
                const r = await fetch(`{{ route("admin.chat.verificar-novas") }}?${params}`);
                const data = await r.json();

                if (this.conversaAtual && data.outro_online !== undefined) {
                    this.conversaAtual.online = data.outro_online;
                }

                if (data.novas_mensagens && data.novas_mensagens.length > 0) {
                    for (const msg of data.novas_mensagens) {
                        if (!this.mensagens.find(m => m.id === msg.id)) {
                ...
                if (this.conversaAtual && data.outro_online) {
                    for (let i = 0; i < this.mensagens.length; i++) {
                        if (this.mensagens[i].minha && !this.mensagens[i].lida) {
                            this.mensagens[i].entregue = true;
                        }
                    }
                }
                ...
                if (!this.conversaAtual && !this.suporteAberto && this.isOpen) this.carregarConversas();
            } catch (e) { console.error(e); }
        },

        scrollToBottom() {
            const c = this.$refs.mensagensContainer;
