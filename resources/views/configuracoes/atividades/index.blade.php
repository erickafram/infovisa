...
<div class="max-w-8xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <p class="text-sm text-gray-600">Cadastre as atividades que pertencem a cada tipo de serviço (Ex: Restaurante, Lanchonete)</p>
        </div>
        <a href="{{ route('admin.configuracoes.atividades.create') }}" 
        ...
        <form method="GET" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="busca" value="{{ request('busca') }}" 
                       placeholder="Buscar por nome, CNAE ou descrição..."
                       class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div class="w-48">
                <select name="tipo_servico_id" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            @if(request()->hasAny(['busca', 'tipo_servico_id', 'status']))
            <a href="{{ route('admin.configuracoes.atividades.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">
                Limpar
            </a>
            @endif
        </form>
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <form action="{{ route('admin.configuracoes.atividades.destroy', $atividade) }}" method="POST" class="inline"
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Excluir">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
