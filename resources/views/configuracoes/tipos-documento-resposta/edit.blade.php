...
@section('title', 'Editar Tipo de Documento Resposta')
@section('page-title', 'Editar Tipo de Documento Resposta')

@section('content')
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.configuracoes.tipos-documento-resposta.update', $tipo) }}" method="POST">
        @csrf @method('PUT')
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="descricao" id="descricao" rows="2"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">{{ old('descricao', $tipo->descricao) }}</textarea>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="tipo_setor" value="todos" {{ old('tipo_setor', $tipo->tipo_setor) === 'todos' ? 'checked' : '' }}
                               class="text-blue-600 focus:ring-blue-500">
                        <span class="text-sm text-gray-700">Todos</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="tipo_setor" value="publico" {{ old('tipo_setor', $tipo->tipo_setor) === 'publico' ? 'checked' : '' }}
                               class="text-blue-600 focus:ring-blue-500">
                        <span class="text-sm text-gray-700">Público</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="tipo_setor" value="privado" {{ old('tipo_setor', $tipo->tipo_setor) === 'privado' ? 'checked' : '' }}
                               class="text-blue-600 focus:ring-blue-500">
