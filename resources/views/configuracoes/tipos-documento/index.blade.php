...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <p class="text-gray-600">Gerencie os tipos de documentos disponíveis no sistema</p>
        </div>
                    ...
                    @foreach($tipos as $tipo)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                ...
                                @if($tipo->descricao)
                                    <div class="text-sm text-gray-500">{{ Str::limit($tipo->descricao, 60) }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <code class="px-2 py-1 bg-gray-100 rounded text-xs">{{ $tipo->codigo }}</code>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.configuracoes.tipos-documento.destroy', $tipo->id) }}" 
                                          method="POST"
                                          onsubmit="return confirm('Tem certeza que deseja remover este tipo? Todos os modelos associados também serão removidos.')">
                                        @csrf
                                        @method('DELETE')
    ...
    @else
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2">Nenhum tipo cadastrado</h3>
            <p class="text-gray-600 mb-6">Comece criando seu primeiro tipo de documento</p>
            <a href="{{ route('admin.configuracoes.tipos-documento.create') }}" 
               class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
