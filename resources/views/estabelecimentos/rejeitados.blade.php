...
<div class="max-w-8xl mx-auto space-y-4">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-900">Rejeitados</h2>
            <p class="text-xs text-gray-400 mt-0.5">Estabelecimentos rejeitados que podem ser reiniciados</p>
        </div>
    ...
    <div class="flex gap-1 bg-gray-100 rounded-lg p-1 text-sm">
        ...
        <a href="{{ route('admin.estabelecimentos.rejeitados') }}" class="flex-1 text-center px-3 py-2 rounded-md font-medium transition bg-white text-red-700 shadow-sm">
            Rejeitados
        ...
        <a href="{{ route('admin.estabelecimentos.desativados') }}" class="flex-1 text-center px-3 py-2 rounded-md font-medium transition text-gray-500 hover:text-gray-700">
            Desativados
    ...
    <form method="GET" action="{{ route('admin.estabelecimentos.rejeitados') }}" class="flex gap-2">
        ...
        @if(request('search'))
            <a href="{{ route('admin.estabelecimentos.rejeitados') }}" class="bg-gray-100 text-gray-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 transition">Limpar</a>
        @endif
    </form>
    @endif
        ...
        <div class="space-y-3">
            @foreach($estabelecimentos as $estabelecimento)
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow"
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-start justify-between gap-3 mb-2">
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                ...
                                <span class="text-[11px] px-2 py-0.5 rounded-full font-medium bg-red-50 text-red-600">Rejeitado</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4 text-xs text-gray-500 mb-2">
                            ...
                            <span class="flex items-center gap-1">
                                ...
                                Rejeitado em {{ $estabelecimento->aprovado_em->format('d/m/Y') }}
                            </span>
                            @endif
                            @if($estabelecimento->aprovadoPor)
                            <span class="text-gray-400">por {{ $estabelecimento->aprovadoPor->nome }}</span>
                            @endif
                        </div>
                        ...
                        <div class="bg-red-50 border border-red-100 rounded-lg px-3 py-2 mb-3">
                            <p class="text-xs text-red-700"><span class="font-semibold">Motivo:</span> {{ $estabelecimento->motivo_rejeicao }}</p>
                        </div>
                        @endif
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button @click="showReiniciar = true"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg transition">
                    ...
                    {{-- Painel Reiniciar (inline) --}}
                    <div x-show="showReiniciar" x-collapse x-cloak class="border-t border-gray-100 bg-amber-50 px-4 py-3">
                        <form action="{{ route('admin.estabelecimentos.reiniciar', $estabelecimento->id) }}" method="POST" class="space-y-2">
                            @csrf
                            <p class="text-xs text-amber-700">O status voltará para "Pendente" para reanálise.</p>
                            <textarea name="observacao" rows="2" placeholder="Observação (opcional)..."
                                      class="w-full px-3 py-2 text-xs border border-amber-200 rounded-lg focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white"></textarea>
                            <div class="flex gap-2">
                                <button type="submit" class="px-3 py-1.5 text-xs font-medium text-white bg-amber-600 hover:bg-amber-700 rounded-lg transition">Confirmar Reinício</button>
                                <button type="button" @click="showReiniciar = false" class="px-3 py-1.5 text-xs font-medium text-gray-600 bg-white border border-gray-200 hover:bg-gray-50 rounded-lg transition">Cancelar</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    @else
        <div class="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <h3 class="text-base font-semibold text-gray-900">Nenhum estabelecimento rejeitado</h3>
            <p class="text-sm text-gray-400 mt-1">Não há estabelecimentos rejeitados no momento</p>
