...
<div class="container-fluid px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-4">
        <div>
            <div class="flex items-center gap-2 mb-1">
                ...
                <h1 class="text-xl font-bold text-gray-900">OS {{ $ordemServico->numero }}</h1>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.ordens-servico.edit', $ordemServico) }}" 
               class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5 text-sm transition-colors">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="px-4 py-3 space-y-3">
                    <div>
                        ...
                        <p class="text-xs text-gray-900 mt-0.5">{{ $ordemServico->estabelecimento->razao_social }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500">Nome Fantasia</label>
                        <p class="text-xs text-gray-900 mt-0.5">{{ $ordemServico->estabelecimento->nome_fantasia }}</p>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="text-xs font-medium text-gray-500">CNPJ</label>
                            <p class="text-xs text-gray-900 mt-0.5">{{ $ordemServico->estabelecimento->cnpj_formatado ?? '-' }}</p>
                        </div>
                        <div>
                            <label class="text-xs font-medium text-gray-500">CEP</label>
                            <p class="text-xs text-gray-900 mt-0.5">{{ $ordemServico->estabelecimento->cep ?? '-' }}</p>
                        </div>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500">Endereço</label>
                        <p class="text-xs text-gray-900 mt-0.5" id="endereco-completo">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label class="text-xs font-medium text-gray-500">Localização</label>
                            <button type="button" 
                        ...
                        <div id="map" class="w-full h-64 rounded-lg border border-gray-300 bg-gray-100 flex items-center justify-center">
                            <div class="text-center text-gray-500">
                                ...
                                <p class="text-xs">Carregando mapa...</p>
                            </div>
                        </div>
            ...
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="px-4 py-3">
                    ...
                    @else
                        <p class="text-sm text-gray-500">Nenhuma ação cadastrada</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="px-4 py-3">
                            ...
                            @foreach($ordemServico->tecnicos() as $tecnico)
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-50 text-green-700 text-xs font-medium rounded-full">
                                    <div class="w-5 h-5 bg-green-100 rounded-full flex items-center justify-center">
                                        <span class="text-green-700 font-semibold text-[10px]">
                    ...
                    @else
                        <p class="text-xs text-gray-500">Nenhum técnico atribuído</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="px-4 py-3">
                    <p class="text-xs text-gray-700 whitespace-pre-wrap leading-relaxed">{{ $ordemServico->observacoes }}</p>
                </div>
            </div>
            @endif
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-lg shadow">
                <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
                    <h2 class="text-sm font-semibold text-gray-900">Informações Gerais</h2>
                </div>
                <div class="px-4 py-3 space-y-3">
                    <div>
                        <label class="text-xs font-medium text-gray-500">Número da OS</label>
                        <p class="text-xs font-semibold text-gray-900 mt-0.5">{{ $ordemServico->numero }}</p>
                    </div>
                    @if($ordemServico->processo)
                    <div>
                        <label class="text-xs font-medium text-gray-500">Processo</label>
                        <p class="text-xs text-gray-900 mt-0.5">{{ $ordemServico->processo->numero_processo }}</p>
                        <p class="text-[10px] text-gray-500">{{ \App\Models\Processo::tipos()[$ordemServico->processo->tipo] ?? $ordemServico->processo->tipo }}</p>
                    </div>
                    @endif
                    <div>
                        <label class="text-xs font-medium text-gray-500">Status</label>
                        <div class="mt-0.5">{!! $ordemServico->status_badge !!}</div>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500">Competência</label>
                        <div class="mt-0.5">{!! $ordemServico->competencia_badge !!}</div>
                    </div>
                    @if($ordemServico->municipio)
                    <div>
                        <label class="text-xs font-medium text-gray-500">Município</label>
                        <p class="text-xs text-gray-900 mt-0.5">{{ $ordemServico->municipio->nome }}/{{ $ordemServico->municipio->uf }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="px-4 py-3 space-y-2.5">
                    <div class="flex justify-between items-center">
                        <label class="text-xs font-medium text-gray-500">Abertura</label>
                        <p class="text-xs text-gray-900">
                            {{ $ordemServico->data_abertura ? $ordemServico->data_abertura->format('d/m/Y') : '-' }}
                        </p>
                    </div>
                    <div class="flex justify-between items-center">
                        <label class="text-xs font-medium text-gray-500">Início</label>
                        <p class="text-xs text-gray-900">
                            {{ $ordemServico->data_inicio ? $ordemServico->data_inicio->format('d/m/Y') : '-' }}
                        </p>
                    </div>
                    <div class="flex justify-between items-center">
                        <label class="text-xs font-medium text-gray-500">Término</label>
                        <p class="text-xs text-gray-900">
                    ...
                    <div class="flex justify-between items-center">
                        <label class="text-xs font-medium text-gray-500">Conclusão</label>
                        <p class="text-xs text-gray-900">
                    ...
                    <div class="flex justify-between items-center pt-2 border-t border-gray-200">
                        <label class="text-xs font-medium text-gray-500">Duração</label>
            ...
            <div class="bg-gray-50 rounded-lg p-3">
                <div class="space-y-1.5 text-[10px] text-gray-500">
                    <div class="flex justify-between items-center">
                        <span>Criado:</span>
                        <span class="font-medium text-gray-700">{{ $ordemServico->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span>Atualizado:</span>
                        <span class="font-medium text-gray-700">{{ $ordemServico->updated_at->format('d/m/Y H:i') }}</span>
                    </div>
                </div>
            </div>
        </div>
