                ...
                <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs font-semibold rounded">
                            {{ $totalDocumentosOs }} {{ $totalDocumentosOs === 1 ? 'documento' : 'documentos' }}
                        </span>
                        @endif
                        @if($temProcessoVinculado && $ordemServico->status === 'em_andamento')
                ...
                <div class="px-5 py-4">
                    ...
                    <div class="space-y-2">
                        ...
                        @php
                            ...
                            $processoDoc = $docOs->processo;
                            $isLoteDoc = $docOs->isLote();
                        @endphp
                        <a href="{{ route('admin.documentos.show', $docOs->id) }}"
                           class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200 hover:bg-indigo-50 hover:border-indigo-200 transition-all group">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="flex-shrink-0 w-8 h-8 rounded-lg {{ $statusDocOs === 'assinado' ? 'bg-green-100' : ($statusDocOs === 'aguardando_assinatura' ? 'bg-yellow-100' : 'bg-gray-100') }} flex items-center justify-center">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-0.5">
                                        ...
                                        @endif
                                        @if($isLoteDoc)
                                        <span class="px-1.5 py-0.5 text-[10px] font-semibold bg-blue-100 text-blue-700 rounded">Lote</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="text-center py-6">
                        ...
                        <p class="text-sm text-gray-500">Nenhum documento criado para esta OS.</p>
                        @if($temProcessoVinculado && $ordemServico->status === 'em_andamento')
                        <a href="{{ route('admin.documentos.create') }}?{{ $processosVinculadosOs->count() > 1 ? 'processos_ids=' . $processosVinculadosOs->implode(',') : 'processo_id=' . $processosVinculadosOs->first() }}&os_id={{ $ordemServico->id }}"
                           class="inline-flex items-center gap-1.5 px-4 py-2 mt-3 text-sm font-semibold text-indigo-700 bg-indigo-50 border-2 border-dashed border-indigo-300 rounded-lg hover:bg-indigo-100 transition-all">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
